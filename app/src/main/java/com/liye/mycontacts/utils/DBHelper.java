...
public class DBHelper extends SQLiteOpenHelper {
	 ...
	 * 方法都可以获取一个用于操作数据库的SQLiteDatabase实例。(getReadableDatabase()
	 * 方法中会调用getWritableDatabase()方法)
	 * 
	 * 其中getWritableDatabase()
	 * 方法以读写方式打开数据库，一旦数据库的磁盘空间满了，数据库就只能读而不能写，倘若使用的是getWritableDatabase() 方法就会出错。
	 * 
	 * getReadableDatabase()方法则是先以读写方式打开数据库，如果数据库的磁盘空间满了，就会打开失败，
	 * 当打开失败后会继续尝试以只读方式打开数据库。如果该问题成功解决，则只读数据库对象就会关闭，然后返回一个可读写的数据库对象。
	 * 
	 * 
	 */
	//// 插入
	...
	// 查询
	// Cursor
	// 一般在sql查询中使用比如你在查询一张表格，而cursor会像手指一样，从第一条开始指向最后一条，这在java中就可以写成一个for循环，然后取出每一条的数据，放入list中，表中的数据就全部拿到了
	public Cursor query() {
		SQLiteDatabase db = getWritableDatabase();
	...
	public ArrayList<NoteEntity> fetchValue(String id) {
		...
		String sql = "select * from Dialy " + "where _id='" + id + "'";
		Log.d("==sql==", sql);
		SQLiteDatabase db = getWritableDatabase();
